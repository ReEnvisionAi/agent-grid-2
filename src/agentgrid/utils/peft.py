...
def get_adapter_from_repo(
    ...
    **kwargs,
    ...
    config_path = cached_file(repo_id, CONFIG_NAME, token=token, **kwargs)
    if config_path is None:
        raise RuntimeError(f"File {CONFIG_NAME} does not exist in repo {repo_id}")
    config = PeftConfig.from_json_file(config_path)

    weight_path = cached_file(repo_id, SAFETENSORS_WEIGHTS_NAME, token=token, **kwargs)
...
def load_peft(
    ...
    *,
    ...
    # TODO: Check is it possible to add safetensors loading inside agentgrid/server/from_pretrained.py and reuse it here
...
class AdapterContextMixin:
    ...
    def set_adapter(self, adapter_names) -> None:
        """
        In PEFT, this function makes the adapter trainable. However, in Agent Grid environment this is not possible now. Thus,
...
def add_adapter_to_block(block, block_index, adapter_name, peft_config, peft_state_dict):
    assert peft_config["peft_type"] == "LORA", "Agent Grid works only with LORA adapters"
