...
from agentgrid.utils.hf_auth import always_needs_auth
...
def _load_state_dict_from_repo(
    ...
    *,
    ...
    if index_file.endswith(".index.json"):  # Sharded model
        path = cached_file(model_name, filename=index_file, token=token, cache_dir=cache_dir)
        if path is None:
...
def _find_index_file(
    ...
    # If we have cached weights (e.g., Pickle from older Agent Grid versions), reuse them
...
def _load_state_dict_from_repo_file(
    ...
    *,
    ...
    try:
        ...
        logger.debug(f"{filename} not found in cache, will be downloaded")
    except Exception:
